@php
    ...
    $isLongColumn = function($column) use($longFields){
        ...
        foreach(['observacion','descripcion','mensaje','detalle','nota','comentario','contenido','direccion'] as $word){
    ...
    $resolveFieldIcon = function($column){
        ...
        if(str_contains($text,'usuario') || str_contains($text,'encargado') || str_contains($text,'registr')) return 'bi-person';
...
<x-app-layout>
    ...
    <style>
        ...
        .crud-table-wrapper::-webkit-scrollbar{height:8px}
        .crud-table-wrapper::-webkit-scrollbar-track{background:#eef7f5;border-radius:10px}
        .crud-table-wrapper::-webkit-scrollbar-thumb{background:#8fbcc3;border-radius:10px}
        .crud-table-wrapper::-webkit-scrollbar-thumb:hover{background:#5c9eaa}
        .crud-table-readable{width:max-content!important;min-width:100%!important;table-layout:auto!important}
        .crud-table-readable th{white-space:nowrap!important;vertical-align:middle;min-width:120px}
        .crud-table-readable td{vertical-align:middle}
        .crud-column-normal{white-space:nowrap!important;min-width:130px!important;max-width:none!important;overflow-wrap:normal!important;word-break:normal!important}
        .crud-column-short{white-space:nowrap!important;min-width:90px!important;width:90px!important;text-align:center}
        .crud-column-medium{white-space:nowrap!important;min-width:160px!important}
        .crud-column-long{white-space:normal!important;min-width:260px!important;max-width:340px!important;line-height:1.45!important;overflow-wrap:break-word!important;word-break:normal!important}
        .crud-column-image{white-space:nowrap!important;min-width:110px!important;width:110px!important;text-align:center}
        .crud-column-actions{white-space:nowrap!important;min-width:120px!important;width:120px!important;text-align:center}
        .crud-cell-content{display:block;width:100%;white-space:inherit;word-break:inherit;overflow-wrap:inherit}
        .crud-image-thumb{width:70px;height:55px;object-fit:cover;display:block}

        /* MODAL VER */
        .crud-view-modal{border:0;background:#fff}
        .crud-view-header{position:relative;padding:24px 28px 15px;text-align:center;background:#fff}
        .crud-view-close{position:absolute;top:15px;right:15px;width:38px;height:38px;display:flex;align-items:center;justify-content:center;border:0;border-radius:12px;background:transparent;color:#88969d;font-size:19px;transition:all .18s ease}
        .crud-view-close:hover{color:#1c607a;background:#eef8f6;transform:rotate(4deg)}
        .crud-view-icon{width:56px;height:56px;display:flex;align-items:center;justify-content:center;margin:0 auto 13px;border-radius:17px;color:#1c607a;background:linear-gradient(145deg,#edf9f7,#e2f4f1);border:1px solid rgba(59,180,156,.24);font-size:25px;box-shadow:0 6px 16px rgba(33,106,115,.07)}
        .crud-view-title{margin:0;color:#173f50;font-size:24px;font-weight:800;letter-spacing:-.025em}
        .crud-view-subtitle{margin:6px auto 0;color:#7b8a93;font-size:14px;font-weight:500}
        .crud-view-body{padding:16px 23px 9px;background:#fff}
        .crud-view-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:15px}
        .crud-view-card{position:relative;min-width:0;min-height:96px;padding:16px 17px;border:1px solid #dce8e8;border-radius:16px;background:linear-gradient(145deg,#fbfdfd,#f7faf9);overflow:hidden;transition:transform .18s ease,border-color .18s ease,box-shadow .18s ease,background .18s ease}
        .crud-view-card::after{content:'';position:absolute;left:0;top:18px;bottom:18px;width:3px;border-radius:0 4px 4px 0;background:transparent;transition:background .18s ease}
        .crud-view-card:hover{transform:translateY(-2px);border-color:rgba(59,180,156,.38);background:#fff;box-shadow:0 8px 22px rgba(22,70,84,.075)}
        .crud-view-card:hover::after{background:#3bb49c}
        .crud-view-card-full{grid-column:1/-1}
        .crud-view-card-header{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:10px}
        .crud-view-label{min-width:0;display:flex;align-items:center;gap:9px;color:#1c607a;font-size:13px;font-weight:750}
        .crud-view-label-text{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
        .crud-view-field-icon{width:29px;height:29px;display:flex;align-items:center;justify-content:center;flex-shrink:0;border-radius:9px;color:#267b86;background:#eaf7f5;border:1px solid rgba(59,180,156,.15);font-size:13px;transition:background .18s ease,color .18s ease,transform .18s ease}
        .crud-view-card:hover .crud-view-field-icon{color:#fff;background:#3b9f94;transform:scale(1.03)}
        .crud-view-index{flex-shrink:0;color:#a1adb2;font-size:9px;font-weight:800;letter-spacing:.08em;line-height:1;padding:4px 6px;border:1px solid #e2eaea;border-radius:6px;background:#fbfcfc;transition:color .18s ease,border-color .18s ease}
        .crud-view-card:hover .crud-view-index{color:#65949a;border-color:#cfe4e1}
        .crud-view-value{padding-left:38px;color:#294753;font-size:15px;font-weight:550;line-height:1.5;white-space:pre-line;overflow-wrap:break-word}
        .crud-view-value-status{display:inline-flex;align-items:center;gap:7px;width:auto;margin-left:38px;padding:6px 11px;border:1px solid rgba(59,180,156,.18);border-radius:999px;color:#226d72;background:#eef9f7;font-size:13px;font-weight:700}
        .crud-view-value-status::before{content:'';width:7px;height:7px;border-radius:999px;background:#3bb49c;box-shadow:0 0 0 3px rgba(59,180,156,.10)}
        .crud-view-empty{padding-left:38px;color:#98a5ac;font-size:14px;font-style:italic}
        .crud-view-image-button{display:block;margin-left:38px;padding:0;border:0;background:transparent;cursor:zoom-in}
        .crud-view-image{display:block;width:155px;height:105px;object-fit:cover;border:1px solid #d7e5e4;border-radius:12px;box-shadow:0 5px 14px rgba(15,23,42,.07);transition:transform .2s ease,box-shadow .2s ease}
        .crud-view-image-button:hover .crud-view-image{transform:scale(1.025);box-shadow:0 9px 22px rgba(15,23,42,.12)}
        .crud-view-footer{display:flex;justify-content:center;padding:19px 24px 24px;background:#fff}
        .crud-view-close-btn{min-width:122px;transition:transform .18s ease,box-shadow .18s ease}
        .crud-view-close-btn:hover{transform:translateY(-1px)}

        /* MODAL IMAGEN */
        .crud-image-modal-content{background:#fff}
        .crud-image-modal-body{padding:22px;text-align:center;background:#f7fbfa}
        .crud-image-modal-preview{max-height:72vh;object-fit:contain;border-radius:14px}

        @media(max-width:767.98px){
            .crud-table-readable th{min-width:110px}
            .crud-column-normal{min-width:125px!important}
            .crud-column-medium{min-width:150px!important}
            .crud-column-long{min-width:230px!important;max-width:280px!important}
            .crud-view-header{padding:21px 18px 12px}
            .crud-view-body{padding:14px 15px 8px}
            .crud-view-grid{grid-template-columns:1fr;gap:12px}
            .crud-view-card-full{grid-column:auto}
            .crud-view-card{min-height:auto;padding:15px}
            .crud-view-title{font-size:21px}
            .crud-view-value,.crud-view-empty{padding-left:38px}
            .crud-view-value-status,.crud-view-image-button{margin-left:38px}
            .crud-view-footer{padding:16px 18px 20px}
        }
    </style>
                ...
                <div class="crud-table-wrapper">
                    ...
                    <table class="table table-hover align-middle mb-0 crud-table-readable">
                        ...
                        <tr>
                            ...
                            @foreach($columns as $column)
                                ...
                                @php
                                    ...
                                    $headerClass = $headerType === 'image' ? 'crud-column-image' : ($headerLong ? 'crud-column-long' : 'crud-column-normal');
                                @endphp
                            ...
                            @if($hasActions)
                                <th class="px-3 px-sm-4 py-3 crud-column-actions">Acciones</th>
                            @endif
                        ...
                        @forelse($items as $item)
                            ...
                            @php
                                $showPayload = collect($columns)->map(function($column) use($item,$formatValue,$formatImageUrl,$resolveFieldIcon,$isLongColumn,$isStatusColumn){
                            ...
                            <tr class="item-row crud-table-row" data-search="{{ $searchText }}">
                                ...
                                @foreach($columns as $column)
                                    ...
                                    @php
                                        ...
                                        if($columnType === 'image'){
                                            $columnClass = 'crud-column-image';
                                        }elseif($isLong){
                                            $columnClass = 'crud-column-long';
                                        }elseif(($column['size'] ?? '') === 'short'){
                                            $columnClass = 'crud-column-short';
                                        }elseif(($column['size'] ?? '') === 'medium'){
                                            $columnClass = 'crud-column-medium';
                                        }
                                    @endphp
                                        ...
                                        <div class="d-inline-flex align-items-center justify-content-center gap-1">
                                            ...
                                            @if($canShow)
                                                ...
                                                @if($showWithRoute)
                                                    ...
                                                    <a href="{{ route($routeBase.'.show',$item) }}" title="Ver datos" class="btn btn-sm btn-outline-primary rounded-2 px-2 py-1">
                                                        <i class="bi bi-eye"></i>
                                                    </a>
                        ...
                        @empty
                            ...
                            <tr>
                                <td colspan="{{ count($columns)+($hasActions ? 1 : 0) }}" class="px-4 py-5 text-center text-muted">
                ...
                <div class="modal-dialog modal-dialog-centered modal-lg modal-dialog-scrollable" @click.outside="closeShowModal()">
                    ...
                    <div class="modal-content shadow-lg rounded-4 overflow-hidden crud-view-modal">
                        ...
                        <div class="crud-view-header">
                            ...
                            <button type="button" class="crud-view-close" @click="closeShowModal()" aria-label="Cerrar">
                                <i class="bi bi-x-lg"></i>
                            </button>

                            <div class="crud-view-icon">
                                <i class="bi bi-eye"></i>
                            </div>
                                ...
                                <template x-for="(field,index) in selectedShow" :key="index">
                                    ...
                                    <div class="crud-view-card" :class="field.type === 'image' || field.full || (field.value && field.value.length > 110) ? 'crud-view-card-full' : ''">
                                        ...
                                        <div class="crud-view-card-header">
                                            ...
                                            <span class="crud-view-field-icon">
                                                <i class="bi" :class="field.icon"></i>
                                            </span>
                                        ...
                                        {{-- ESTADO --}}
                                        <template x-if="field.type !== 'image' && field.status">
                        ...
                        <div class="crud-view-footer">
                            ...
                            <button type="button" class="btn btn-brand-outline crud-view-close-btn px-4 py-2 rounded-3" @click="closeShowModal()">
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg" @click.outside="closeImageModal()">
                ...
                <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden crud-image-modal-content">
                    ...
                    <div class="d-flex justify-content-between align-items-center px-4 py-3 border-bottom">
                        ...
                        <div>
                            <h5 class="fw-bold text-brand-dark mb-0">Vista de fotografía</h5>
                            <small class="text-secondary">Evidencia registrada en el sistema</small>
                        </div>
                    ...
                    <div class="crud-image-modal-body">
                        <img :src="selectedImage" alt="Fotografía de evidencia" class="img-fluid crud-image-modal-preview">
                    </div>

                    <div class="modal-footer border-top-0 justify-content-end px-4 pb-4 pt-2">
                        <button type="button" class="btn btn-brand-outline px-4 py-2 rounded-3" @click="closeImageModal()">
                            <i class="bi bi-x-circle me-2"></i>Cerrar
                        </button>
                    </div>
    ...
    <script>
        function crudModule(){
            return {
                ...
                resetFileInputs(){
                    this.$nextTick(()=>{
                        document.querySelectorAll('.microseed-file-input').forEach(input=>input.value='');
                    });
                },
