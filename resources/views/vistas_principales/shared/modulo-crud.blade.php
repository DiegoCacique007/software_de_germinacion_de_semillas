@php
    ...
    $hasActions=$canShow||$canUpdate||$canDestroy||collect($quickActions)->isNotEmpty();
...
<x-app-layout>
    ...
    <style>
        ...
        .crud-column-image{white-space:nowrap!important;min-width:110px!important;width:110px!important;text-align:center}.crud-column-actions{white-space:nowrap!important;min-width:145px!important;width:145px!important;text-align:center}.crud-cell-content{display:block;width:100%;white-space:inherit;word-break:inherit;overflow-wrap:inherit}.crud-image-thumb{width:70px;height:55px;object-fit:cover;display:block}
        ...
        .crud-view-title{margin:0;color:#173f50;font-size:24px;font-weight:800;letter-spacing:-.025em}.crud-view-subtitle{margin:6px auto 0;color:#7b8a93;font-size:14px;font-weight:500}
        .crud-view-body{padding:16px 23px 9px;background:#fff}.crud-view-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:15px}.crud-view-card{position:relative;min-width:0;min-height:96px;padding:16px 17px;border:1px solid #dce8e8;border-radius:16px;background:linear-gradient(145deg,#fbfdfd,#f7faf9);overflow:hidden;transition:.18s}
        .crud-view-card::after{content:'';position:absolute;left:0;top:18px;bottom:18px;width:3px;border-radius:0 4px 4px 0;background:transparent;transition:.18s}.crud-view-card:hover{transform:translateY(-2px);border-color:rgba(59,180,156,.38);background:#fff;box-shadow:0 8px 22px rgba(22,70,84,.075)}.crud-view-card:hover::after{background:#3bb49c}.crud-view-card-full{grid-column:1/-1}
    ...
    <div class="container-fluid py-4 px-3 px-lg-4" x-data="crudModule()" x-init="init()" x-cloak>
        ...
        @if($errors->any())
            <div class="alert alert-danger shadow-sm rounded-3 mb-4">
                <div class="d-flex align-items-center gap-2 mb-2 fw-bold">
                    <i class="bi bi-exclamation-triangle-fill fs-5"></i><span>Corrige los siguientes errores:</span>
                </div>
                ...
                <div class="crud-table-wrapper">
                    ...
                    <table class="table table-hover align-middle mb-0 crud-table-readable">
                        ...
                        <tr>
                            @foreach($columns as $column)
                                @php
                                    ...
                                    $headerClass=$headerType==='image'
                                        ?'crud-column-image'
                                        :($headerLong
                                            ?'crud-column-long'
                                            :(($column['size']??'')==='short'
                                                ?'crud-column-short'
                                                :(($column['size']??'')==='medium'?'crud-column-medium':'crud-column-normal')));
                                @endphp
                        ...
                        @forelse($items as $item)
                            ...
                            @php
                                $showPayload=collect($columns)->map(function($column)use(
                                    $item,$formatValue,$formatImageUrl,$resolveFieldIcon,$isLongColumn,$isStatusColumn,$isLevelColumn
                                ){
        ...
        <i class="bi {{ $levelIcon($displayValue) }} me-1"></i>{{ $displayValue }}
                                        ...
                                        <div class="d-inline-flex align-items-center justify-content-center gap-1">
                                            ...
                                            @foreach($quickActions as $action)
                                                ...
                                                @php
                                                    ...
                                                    if($when){
                                                        $current=(string)data_get($item,$when['key']??'');
                                                        $allowed=array_map('strval',$when['values']??[]);
                                                        $visible=in_array($current,$allowed,true);
                                                    }

                                                    $actionMethod=strtoupper($action['method']??'POST');
                                                    $actionRoute=$action['route']??null;
                                                @endphp

                                                @if($visible&&$actionRoute&&Route::has($actionRoute))

                                                    <form action="{{ route($actionRoute,$item) }}" method="POST" class="d-inline" @submit.prevent="askQuickActionConfirmation($event)">
                                                        @csrf
                                                        ...
                                                        <button type="submit"
                                                                ...
                                                                class="btn btn-sm {{ $action['class']??'btn-outline-primary' }} rounded-2 px-2 py-1">
                                                            <i class="bi {{ $action['icon']??'bi-check-lg' }}"></i>
                                                        </button>
                                                    </form>
            ...
            <div x-show="rows.length>0&&totalPages>1"
                 x-cloak
    ...
    <span class="small text-secondary">
        Página <strong class="text-brand-primary" x-text="currentPage"></strong>
        de <strong class="text-brand-primary" x-text="totalPages"></strong>
    </span>
                        ...
                        <div class="modal-body crud-view-body">
                            ...
                            <div class="crud-view-grid">
                                ...
                                <template x-for="(field,index) in selectedShow" :key="index">
                                    ...
                                    <div class="crud-view-card"
                                         :class="field.type==='image'||field.full||(field.value&&field.value.length>110)?'crud-view-card-full':''">
        ...
        <span class="crud-view-field-icon">
            <i class="bi" :class="field.icon"></i>
                                        ...
                                        <template x-if="field.type==='image'&&field.url">
                                            ...
                                            <button type="button"
                                                    class="crud-view-image-button"
                                                    @click="openImageModal(field.url)"
                        ...
                        <div class="crud-view-footer">
                            ...
                            <button type="button"
                                    class="btn btn-brand-outline crud-view-close-btn px-4 py-2 rounded-3"
        ...
        <div x-show.important="imageModalOpen"
             x-cloak
             class="modal fade show d-block crud-modal-backdrop"
             tabindex="-1"
             role="dialog"
                ...
                <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden crud-image-modal-content">
                    ...
                    <div class="crud-image-modal-body">
                        ...
                        <img :src="selectedImage"
                             alt="Fotografía de evidencia"
                             class="img-fluid crud-image-modal-preview">
                    ...
                    <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden position-relative">
                        ...
                        <div class="modal-header border-bottom-0 pb-0 pt-4 px-4 text-center d-flex flex-column align-items-center">
                            ...
                            <h4 class="modal-title fw-bold text-brand-dark"
                                x-text="formMode==='create'?'Nuevo {{ $entitySingular }}':'Editar {{ $entitySingular }}'"></h4>

                            <p class="text-secondary small mb-0 mt-1"
                               x-text="formMode==='create'?'Ingresa la información necesaria para registrar un nuevo elemento.':'Modifica la información del registro seleccionado.'"></p>
                                        ...
                                        <div class="{{ $fullWidth?'col-12':'col-12 col-md-6' }}">
                                            ...
                                            @if($fieldType==='textarea')
                                                ...
                                                <textarea id="{{ $fieldName }}"
                                                          ...
                                                          rows="{{ $field['rows']??3 }}"
                                                          @if($required) required @elseif($requiredCreate) x-bind:required="formMode==='create'" @endif
                                            ...
                                            @elseif($fieldType==='file')
                                                ...
                                                <input id="{{ $fieldName }}"
                                                       ...
                                                       @if($required) required @elseif($requiredCreate) x-bind:required="formMode==='create'" @endif
                                                       class="microseed-file-input form-control rounded-3 @error($fieldName) is-invalid @enderror">
    ...
    <script>
        function crudModule(){
            return{
                ...
                async askQuickActionConfirmation(event){
