        ...
        <div class="flex-1 w-full">
            <div
                x-data="{
                    ...
                    loadEdit(data) {
                        this.evidenciaToEdit = {
                            id: data.id || '',
                            seguimiento_lote_id: data.seguimiento || '',
            ...
            >
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 animate-fade-up">
                    <div>
                        <h2 class="text-3xl font-extrabold text-[#1c607a]">Evidencias de Lote</h2>
                        <p class="text-gray-500 mt-1">Sube y administra archivos asociados a seguimientos de lote.</p>
                    </div>
                ...
                <div class="bg-white rounded-2xl shadow-[0_15px_40px_-15px_rgba(28,96,122,0.15)] border border-gray-100 overflow-hidden animate-fade-up">
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-100">
                            ...
                            <tr>
                                <th class="px-6 py-4 text-left text-xs font-black text-[#1c607a] uppercase tracking-wider">Seguimiento</th>
                                <th class="px-6 py-4 text-left text-xs font-black text-[#1c607a] uppercase tracking-wider">Archivo</th>
                                <th class="px-6 py-4 text-left text-xs font-black text-[#1c607a] uppercase tracking-wider">Descripción</th>
                            ...
                            @forelse($evidencias as $evidencia)
                                <tr class="hover:bg-[#3bb49c]/5 transition-colors duration-200 group">
                                    <td class="px-6 py-4 text-sm font-bold text-gray-800">
                                        {{ $evidencia->seguimiento->lote->codigo_lote ?? 'Sin lote' }}
                                        @if($evidencia->seguimiento?->lote?->especie?->nombre_comun)
                                            <div class="text-xs text-gray-500 font-normal">
                                                {{ $evidencia->seguimiento->lote->especie->nombre_comun }}
                                            </div>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-700">
                                        <a href="{{ asset('storage/' . $evidencia->archivo) }}"
                                           target="_blank"
                                           class="text-[#1c607a] font-semibold underline">
                                            Ver archivo
                                        </a>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-500">{{ $evidencia->descripcion }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        <div class="flex items-center justify-center gap-3 opacity-80 group-hover:opacity-100 transition-opacity">
                                            <button
                                                ...
                                                data-seguimiento="{{ $evidencia->seguimiento_lote_id }}"
                                                data-descripcion="{{ $evidencia->descripcion }}"
                                                data-archivo="{{ $evidencia->archivo }}"
                                                data-action="{{ route('encargado.evidencias-lote.update', $evidencia->id) }}"
                                            ...
                                            <form action="{{ route('encargado.evidencias-lote.destroy', $evidencia->id) }}" method="POST" class="inline m-0">
                                                @csrf
                                                @method('DELETE')
                                                <button
                                                    type="submit"
                                                    onclick="return confirm('¿Deseas eliminar esta evidencia?')"
                ...
                <div x-show="openCreate" class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center glass-modal p-4">
                    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden" @click.away="openCreate = false">
                        <div class="bg-gradient-to-r from-[#1c607a] to-[#3bb49c] p-6 text-white text-center">
                            <h3 class="text-2xl font-extrabold">Nueva Evidencia</h3>
                            <p class="text-sm opacity-90 mt-1">Sube un archivo para un seguimiento de lote.</p>
                        </div>

                        <form action="{{ route('encargado.evidencias-lote.store') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-5">
                            @csrf

                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Seguimiento de lote</label>
                                <select name="seguimiento_lote_id" required class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700 bg-white">
                                    <option value="">Seleccione un seguimiento</option>
                                    @foreach($seguimientos as $seguimiento)
                                        <option value="{{ $seguimiento->id }}">
                                            Lote {{ $seguimiento->lote->codigo_lote ?? 'Sin lote' }} - {{ $seguimiento->fecha_revision }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Archivo</label>
                                <input type="file" name="archivo" required class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700 bg-white">
                            </div>
                            ...
                            <div class="flex justify-end gap-3 pt-6 border-t border-gray-100 mt-2">
                                ...
                                <button type="submit" class="btn-brand text-white px-6 py-2.5 rounded-xl font-bold">Registrar Evidencia</button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div x-show="openEdit" class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center glass-modal p-4">
                    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden" @click.away="closeEdit()">
                        <div class="bg-[#f0f6f6] p-6 border-b border-gray-200">
                            <h3 class="text-2xl font-extrabold text-[#1c607a]">Editar Evidencia</h3>
                            <p class="text-sm text-gray-500 mt-1">Modifica la evidencia seleccionada.</p>
                        </div>

                        <form :action="editAction" method="POST" enctype="multipart/form-data" class="p-8 space-y-5">
                            @csrf
                            @method('PUT')

                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Seguimiento de lote</label>
                                <select name="seguimiento_lote_id" x-model="evidenciaToEdit.seguimiento_lote_id" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700 bg-white">
                                    <option value="">Seleccione un seguimiento</option>
                                    @foreach($seguimientos as $seguimiento)
                                        <option value="{{ $seguimiento->id }}">
                                            Lote {{ $seguimiento->lote->codigo_lote ?? 'Sin lote' }} - {{ $seguimiento->fecha_revision }}
                            ...
                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Reemplazar archivo (opcional)</label>
                                <input type="file" name="archivo" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700 bg-white">
                            </div>

                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Descripción</label>
                                <textarea name="descripcion" x-model="evidenciaToEdit.descripcion" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700"></textarea>
                            </div>
