        ...
        <div class="flex-1 w-full">
                ...
                x-data="{
                    ...
                    loadEdit(data) {
                        this.alertaToEdit = {
                            ...
                            tipo_alerta_id: data.tipo || '',
                            nivel_alerta_id: data.nivel || '',
                            estado_alerta_id: data.estado || '',
            ...
            >
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 animate-fade-up">
                    <div>
                        <h2 class="text-3xl font-extrabold text-[#1c607a]">Alertas</h2>
                        <p class="text-gray-500 mt-1">Registra y administra alertas operativas del sistema.</p>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-100">
                            ...
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-black text-[#1c607a] uppercase tracking-wider">Nivel</th>
                                <th class="px-6 py-4 text-left text-xs font-black text-[#1c607a] uppercase tracking-wider">Estado</th>
                                <th class="px-6 py-4 text-left text-xs font-black text-[#1c607a] uppercase tracking-wider">Mensaje</th>
                                <th class="px-6 py-4 text-left text-xs font-black text-[#1c607a] uppercase tracking-wider">Fecha y hora</th>
                                <th class="px-6 py-4 text-left text-xs font-black text-[#1c607a] uppercase tracking-wider">Atendida por</th>
                            ...
                            @forelse($alertas as $alerta)
                                <tr class="hover:bg-[#3bb49c]/5 transition-colors duration-200 group">
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-700">{{ $alerta->tipo->nombre ?? 'Sin tipo' }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-700">{{ $alerta->nivel->nombre ?? 'Sin nivel' }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-700">{{ $alerta->estado->nombre ?? 'Sin estado' }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-700">{{ $alerta->mensaje }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-700">
                                        {{ \Carbon\Carbon::parse($alerta->fecha_hora)->format('Y-m-d\TH:i') }}
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-500">{{ $alerta->atendidaPor->name ?? 'Sin asignar' }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        <div class="flex items-center justify-center gap-3 opacity-80 group-hover:opacity-100 transition-opacity">
                                            <button
                                                type="button"
                                                data-id="{{ $alerta->id }}"
                                                data-incubadora="{{ $alerta->incubadora_id }}"
                                                data-tipo="{{ $alerta->tipo_alerta_id }}"
                                                data-nivel="{{ $alerta->nivel_alerta_id }}"
                                                data-estado="{{ $alerta->estado_alerta_id }}"
                                                data-mensaje="{{ $alerta->mensaje }}"
                                                data-fechahora="{{ \Carbon\Carbon::parse($alerta->fecha_hora)->format('Y-m-d\TH:i') }}"
                                                data-observaciones="{{ $alerta->observaciones }}"
                                                data-action="{{ route('encargado.alertas.update', $alerta->id) }}"
                                            ...
                                            <form action="{{ route('encargado.alertas.destroy', $alerta->id) }}" method="POST" class="inline m-0">
                                                @csrf
                                                @method('DELETE')
                                                <button
                                                    type="submit"
                                                    onclick="return confirm('¿Deseas eliminar esta alerta?')"
                ...
                <div x-show="openCreate" class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center glass-modal p-4">
                    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden" @click.away="openCreate = false">
                        <div class="bg-gradient-to-r from-[#1c607a] to-[#3bb49c] p-6 text-white text-center">
                            <h3 class="text-2xl font-extrabold">Nueva Alerta</h3>
                            <p class="text-sm opacity-90 mt-1">Registra una alerta operativa.</p>
                        </div>

                        <form action="{{ route('encargado.alertas.store') }}" method="POST" class="p-8 space-y-5">
                            @csrf
                            ...
                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Tipo de alerta</label>
                                <select name="tipo_alerta_id" required class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700 bg-white">
                            ...
                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Nivel de alerta</label>
                                <select name="nivel_alerta_id" required class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700 bg-white">
                            ...
                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Estado de alerta</label>
                                <select name="estado_alerta_id" required class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700 bg-white">
                            ...
                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Mensaje</label>
                                <input type="text" name="mensaje" required class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700">
                            </div>
                            ...
                            <div class="flex justify-end gap-3 pt-6 border-t border-gray-100 mt-2">
                                ...
                                <button type="submit" class="btn-brand text-white px-6 py-2.5 rounded-xl font-bold">Registrar Alerta</button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div x-show="openEdit" class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center glass-modal p-4">
                    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden" @click.away="closeEdit()">
                        <div class="bg-[#f0f6f6] p-6 border-b border-gray-200">
                            <h3 class="text-2xl font-extrabold text-[#1c607a]">Editar Alerta</h3>
                            <p class="text-sm text-gray-500 mt-1">Modifica la alerta seleccionada.</p>
                        </div>
                        ...
                        <form :action="editAction" method="POST" class="p-8 space-y-5">
                            ...
                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Incubadora</label>
                                <select name="incubadora_id" x-model="alertaToEdit.incubadora_id" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700 bg-white">
                                    <option value="">Seleccione una incubadora</option>
                                    @foreach($incubadoras as $incubadora)
                                        <option value="{{ $incubadora->id }}">{{ $incubadora->nombre }} ({{ $incubadora->codigo }})</option>
                                    @endforeach
                            ...
                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Tipo de alerta</label>
                                <select name="tipo_alerta_id" x-model="alertaToEdit.tipo_alerta_id" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700 bg-white">
                                    <option value="">Seleccione un tipo</option>
                                    @foreach($tipos as $tipo)
                                        <option value="{{ $tipo->id }}">{{ $tipo->nombre }}</option>
                                    @endforeach
                            ...
                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Nivel de alerta</label>
                                <select name="nivel_alerta_id" x-model="alertaToEdit.nivel_alerta_id" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700 bg-white">
                                    <option value="">Seleccione un nivel</option>
                                    @foreach($niveles as $nivel)
                                        <option value="{{ $nivel->id }}">{{ $nivel->nombre }}</option>
                                    @endforeach
                            ...
                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Estado de alerta</label>
                                <select name="estado_alerta_id" x-model="alertaToEdit.estado_alerta_id" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700 bg-white">
                                    <option value="">Seleccione un estado</option>
                                    @foreach($estados as $estado)
                                        <option value="{{ $estado->id }}">{{ $estado->nombre }}</option>
                                    @endforeach
                            ...
                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Mensaje</label>
                                <input type="text" name="mensaje" x-model="alertaToEdit.mensaje" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700">
                            </div>

                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Fecha y hora</label>
                                <input type="datetime-local" name="fecha_hora" x-model="alertaToEdit.fecha_hora" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700">
                            </div>

                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Observaciones</label>
                                <textarea name="observaciones" x-model="alertaToEdit.observaciones" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700"></textarea>
                            </div>
