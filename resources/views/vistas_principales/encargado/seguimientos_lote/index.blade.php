        ...
        <div class="flex-1 w-full">
                ...
                x-data="{
                    ...
                    loadEdit(data) {
                        this.seguimientoToEdit = {
                            ...
                            frascos_activos: data.frascosactivos || '',
                            semillas_germinadas: data.semillasgerminadas || '',
                            porcentaje_germinacion: data.porcentajegerminacion || '',
                            altura_promedio_cm: data.alturapromedio || '',
                            etapa_desarrollo_id: data.etapa || '',
            ...
            >
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 animate-fade-up">
                    <div>
                        <h2 class="text-3xl font-extrabold text-[#1c607a]">Seguimientos de Lote</h2>
                        <p class="text-gray-500 mt-1">Registra el avance general de cada lote en revisión.</p>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-100">
                            ...
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-black text-[#1c607a] uppercase tracking-wider">Frascos activos</th>
                                <th class="px-6 py-4 text-left text-xs font-black text-[#1c607a] uppercase tracking-wider">Germinadas</th>
                                <th class="px-6 py-4 text-left text-xs font-black text-[#1c607a] uppercase tracking-wider">% Germinación</th>
                                <th class="px-6 py-4 text-left text-xs font-black text-[#1c607a] uppercase tracking-wider">Etapa</th>
                            ...
                            @forelse($seguimientos as $seguimiento)
                                <tr class="hover:bg-[#3bb49c]/5 transition-colors duration-200 group">
                                    <td class="px-6 py-4 text-sm font-bold text-gray-800">
                                        ...
                                        @if($seguimiento->lote?->especie?->nombre_comun)
                                            <div class="text-xs text-gray-500 font-normal">{{ $seguimiento->lote->especie->nombre_comun }}</div>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-700">{{ $seguimiento->fecha_revision }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-700">{{ $seguimiento->frascos_activos }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-700">{{ $seguimiento->semillas_germinadas }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-700">{{ $seguimiento->porcentaje_germinacion }} %</td>
                                    <td class="px-6 py-4 text-sm text-gray-700">{{ $seguimiento->etapa->nombre ?? 'Sin etapa' }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        <div class="flex items-center justify-center gap-3 opacity-80 group-hover:opacity-100 transition-opacity">
                                            <button
                                                ...
                                                data-lote="{{ $seguimiento->lote_id }}"
                                                data-fecharevision="{{ $seguimiento->fecha_revision }}"
                                                data-frascosactivos="{{ $seguimiento->frascos_activos }}"
                                                data-semillasgerminadas="{{ $seguimiento->semillas_germinadas }}"
                                                data-porcentajegerminacion="{{ $seguimiento->porcentaje_germinacion }}"
                                                data-alturapromedio="{{ $seguimiento->altura_promedio_cm }}"
                                                data-etapa="{{ $seguimiento->etapa_desarrollo_id }}"
                                                data-observaciones="{{ $seguimiento->observaciones }}"
                                                data-action="{{ route('encargado.seguimientos-lote.update', $seguimiento->id) }}"
                                            ...
                                            <form action="{{ route('encargado.seguimientos-lote.destroy', $seguimiento->id) }}" method="POST" class="inline m-0">
                                                @csrf
                                                @method('DELETE')
                                                <button
                                                    type="submit"
                                                    onclick="return confirm('¿Deseas eliminar este seguimiento de lote?')"
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-8 text-center text-gray-500">
                                        No hay seguimientos de lote registrados.
                                    </td>
                                </tr>
                            @endforelse
                            </tbody>
                ...
                <div x-show="openCreate" class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center glass-modal p-4">
                    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden" @click.away="openCreate = false">
                        <div class="bg-gradient-to-r from-[#1c607a] to-[#3bb49c] p-6 text-white text-center">
                            ...
                            <p class="text-sm opacity-90 mt-1">Registra una revisión general del lote.</p>
                        </div>

                        <form action="{{ route('encargado.seguimientos-lote.store') }}" method="POST" class="p-8 space-y-5">
                            @csrf
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-bold text-[#1c607a] mb-1">Frascos activos</label>
                                    <input type="number" name="frascos_activos" min="0" max="255" required class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700">
                                </div>
                                <div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-bold text-[#1c607a] mb-1">% Germinación</label>
                                    <input type="number" step="0.01" name="porcentaje_germinacion" min="0" max="100" required class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700">
                                </div>
                                <div>
                            ...
                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Etapa de desarrollo</label>
                                <select name="etapa_desarrollo_id" required class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700 bg-white">
                ...
                <div x-show="openEdit" class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center glass-modal p-4">
                    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden" @click.away="closeEdit()">
                        <div class="bg-[#f0f6f6] p-6 border-b border-gray-200">
                            ...
                            <p class="text-sm text-gray-500 mt-1">Modifica la revisión del lote.</p>
                        </div>
                        ...
                        <form :action="editAction" method="POST" class="p-8 space-y-5">
                            ...
                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Lote</label>
                                <select name="lote_id" x-model="seguimientoToEdit.lote_id" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700 bg-white">
                                    <option value="">Seleccione un lote</option>
                                    @foreach($lotes as $lote)
                                        <option value="{{ $lote->id }}">
                                            {{ $lote->codigo_lote }} - {{ $lote->especie->nombre_comun ?? 'Sin especie' }}
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-bold text-[#1c607a] mb-1">Frascos activos</label>
                                    <input type="number" name="frascos_activos" min="0" max="255" x-model="seguimientoToEdit.frascos_activos" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700">
                                </div>
                                <div>
                                    <label class="block text-sm font-bold text-[#1c607a] mb-1">Semillas germinadas</label>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <label class="block text-sm font-bold text-[#1c607a] mb-1">% Germinación</label>
                                    <input type="number" step="0.01" name="porcentaje_germinacion" min="0" max="100" x-model="seguimientoToEdit.porcentaje_germinacion" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700">
                                </div>
                                <div>
                                    <label class="block text-sm font-bold text-[#1c607a] mb-1">Altura promedio (cm)</label>
                            ...
                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Etapa de desarrollo</label>
                                <select name="etapa_desarrollo_id" x-model="seguimientoToEdit.etapa_desarrollo_id" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700 bg-white">
                                    <option value="">Seleccione una etapa</option>
                                    @foreach($etapas as $etapa)
                                        <option value="{{ $etapa->id }}">{{ $etapa->nombre }}</option>
                                    @endforeach
