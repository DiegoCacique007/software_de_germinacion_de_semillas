        ...
        <div class="flex-1 w-full">
            <div
                x-data="{
                    openCreate: false,
                    openEdit: false,
                    posicionToEdit: { id: '', incubadora_id: '', numero_posicion: '', descripcion: '' },
                    editAction: '',
                    ...
                    closeEdit() {
                        this.openEdit = false;
                        this.posicionToEdit = { id: '', incubadora_id: '', numero_posicion: '', descripcion: '' };
                        this.editAction = '';
                    }
                }"
                x-cloak
            ...
            >
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 animate-fade-up">
                    <div>
                        <h2 class="text-3xl font-extrabold text-[#1c607a]">Posiciones de Incubadora</h2>
                        <p class="text-gray-500 mt-1">Administra las posiciones disponibles dentro de cada incubadora.</p>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-100">
                            ...
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-black text-[#1c607a] uppercase tracking-wider">Número de Posición</th>
                            ...
                            @forelse($posiciones as $posicion)
                                <tr class="hover:bg-[#3bb49c]/5 transition-colors duration-200 group">
                                    <td class="px-6 py-4 text-sm font-bold text-gray-800">{{ $posicion->incubadora->nombre ?? 'Sin incubadora' }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-700">{{ $posicion->numero_posicion }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-500">{{ $posicion->descripcion }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        <div class="flex items-center justify-center gap-3 opacity-80 group-hover:opacity-100 transition-opacity">
                                            <button
                                                type="button"
                                                data-id="{{ $posicion->id }}"
                                                data-incubadora="{{ $posicion->incubadora_id }}"
                                                data-numero="{{ $posicion->numero_posicion }}"
                                                data-descripcion="{{ $posicion->descripcion }}"
                                                data-action="{{ route('administrador.posiciones-incubadora.update', $posicion->id) }}"
                                            ...
                                            <form action="{{ route('administrador.posiciones-incubadora.destroy', $posicion->id) }}" method="POST" class="inline m-0">
                                                @csrf
                                                @method('DELETE')
                                                <button
                                                    type="submit"
                                                    onclick="return confirm('¿Deseas eliminar esta posición?')"
                ...
                <div x-show="openCreate"
                    ...
                    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden"
                        ...
                        <div class="bg-gradient-to-r from-[#1c607a] to-[#3bb49c] p-6 text-white text-center">
                            <h3 class="text-2xl font-extrabold">Nueva Posición</h3>
                            <p class="text-sm opacity-90 mt-1">Registra una posición dentro de una incubadora.</p>
                        </div>

                        <form action="{{ route('administrador.posiciones-incubadora.store') }}" method="POST" class="p-8 space-y-5">
                            @csrf
                            ...
                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Número de posición</label>
                                <input type="number" name="numero_posicion" min="1" max="5" required class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700">
                            </div>
                ...
                <div x-show="openEdit"
                    ...
                    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden"
                        ...
                        <div class="bg-[#f0f6f6] p-6 border-b border-gray-200">
                            <h3 class="text-2xl font-extrabold text-[#1c607a]">Editar Posición</h3>
                            <p class="text-sm text-gray-500 mt-1">Modifica la posición seleccionada.</p>
                        </div>
                        ...
                        <form :action="editAction" method="POST" class="p-8 space-y-5">
                            ...
                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Incubadora</label>
                                <select name="incubadora_id" x-model="posicionToEdit.incubadora_id" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700 bg-white">
                                    <option value="">Seleccione una incubadora</option>
                                    @foreach($incubadoras as $incubadora)
                                        <option value="{{ $incubadora->id }}">{{ $incubadora->nombre }} ({{ $incubadora->codigo }})</option>
                                    @endforeach
                            ...
                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Número de posición</label>
                                <input type="number" name="numero_posicion" min="1" max="5" x-model="posicionToEdit.numero_posicion" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700">
                            </div>

                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Descripción</label>
                                <textarea name="descripcion" x-model="posicionToEdit.descripcion" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700"></textarea>
                            </div>
