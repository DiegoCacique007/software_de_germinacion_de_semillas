        ...
        <div class="flex-1 w-full">
            <div
                x-data="{
                    openCreate: false,
                    openEdit: false,
                    estadoToEdit: { id: '', clave: '', nombre: '', descripcion: '' },
                    editAction: '',
                    ...
                    closeEdit() {
                        this.openEdit = false;
                        this.estadoToEdit = { id: '', clave: '', nombre: '', descripcion: '' };
                        this.editAction = '';
                    }
                }"
                x-cloak
            ...
            >
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 animate-fade-up">
                    <div>
                        <h2 class="text-3xl font-extrabold text-[#1c607a]">Estados de Incubadora</h2>
                        <p class="text-gray-500 mt-1">Administra el catálogo de estados disponibles para las incubadoras.</p>
                    </div>
                ...
                <div class="bg-white rounded-2xl shadow-[0_15px_40px_-15px_rgba(28,96,122,0.15)] border border-gray-100 overflow-hidden animate-fade-up">
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-100">
                            ...
                            <tr>
                                <th class="px-6 py-4 text-left text-xs font-black text-[#1c607a] uppercase tracking-wider">Clave</th>
                            ...
                            @forelse($estados as $estado)
                                <tr class="hover:bg-[#3bb49c]/5 transition-colors duration-200 group">
                                    <td class="px-6 py-4 text-sm font-bold text-gray-800">{{ $estado->clave }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-700">{{ $estado->nombre }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        <div class="flex items-center justify-center gap-3 opacity-80 group-hover:opacity-100 transition-opacity">
                                            <button
                                                ...
                                                data-action="{{ route('super_admin.estados-incubadora.update', $estado->id) }}"
                                            ...
                                            <form action="{{ route('super_admin.estados-incubadora.destroy', $estado->id) }}" method="POST" class="inline m-0">
                                                @csrf
                                                @method('DELETE')
                                                <button
                                                    type="submit"
                                                    onclick="return confirm('¿Deseas eliminar este estado?')"
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-6 py-8 text-center text-gray-500">
                                        No hay estados de incubadora registrados.
                                    </td>
                                </tr>
                            @endforelse
                            </tbody>
                ...
                <div x-show="openCreate"
                    ...
                    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden"
                        ...
                        <div class="bg-gradient-to-r from-[#1c607a] to-[#3bb49c] p-6 text-white text-center">
                            <h3 class="text-2xl font-extrabold">Nuevo Estado</h3>
                            <p class="text-sm opacity-90 mt-1">Registra un nuevo estado de incubadora.</p>
                        </div>

                        <form action="{{ route('super_admin.estados-incubadora.store') }}" method="POST" class="p-8 space-y-5">
                            @csrf

                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Clave</label>
                                <input type="text" name="clave" required class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700">
                            </div>
                ...
                <div x-show="openEdit"
                    ...
                    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden"
                        ...
                        <div class="bg-[#f0f6f6] p-6 border-b border-gray-200">
                            <h3 class="text-2xl font-extrabold text-[#1c607a]">Editar Estado</h3>
                            <p class="text-sm text-gray-500 mt-1">Modifica la información del estado.</p>
                        </div>
                        ...
                        <form :action="editAction" method="POST" class="p-8 space-y-5">
                            ...
                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Clave</label>
                                <input type="text" name="clave" x-model="estadoToEdit.clave" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700">
                            </div>

                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Nombre</label>
                                <input type="text" name="nombre" x-model="estadoToEdit.nombre" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700">
                            </div>

                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Descripción</label>
                                <textarea name="descripcion" x-model="estadoToEdit.descripcion" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700"></textarea>
                            </div>
