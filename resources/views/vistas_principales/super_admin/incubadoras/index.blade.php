        ...
        <div class="flex-1 w-full">
            <div
                x-data="{
                    openCreate: false,
                    openEdit: false,
                    incubadoraToEdit: { id: '', codigo: '', nombre: '', ubicacion: '', descripcion: '', estado_incubadora_id: '' },
                    editAction: '',

                    loadEdit(data) {
                        this.incubadoraToEdit = {
                            id: data.id || '',
                            codigo: data.codigo || '',
                            nombre: data.nombre || '',
                            ubicacion: data.ubicacion || '',
                            descripcion: data.descripcion || '',
                            estado_incubadora_id: data.estado || ''
                        };
                    ...
                    closeEdit() {
                        this.openEdit = false;
                        this.incubadoraToEdit = { id: '', codigo: '', nombre: '', ubicacion: '', descripcion: '', estado_incubadora_id: '' };
                        this.editAction = '';
                    }
                }"
                x-cloak
            ...
            >
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 animate-fade-up">
                    <div>
                        <h2 class="text-3xl font-extrabold text-[#1c607a]">Incubadoras</h2>
                        <p class="text-gray-500 mt-1">Administra las incubadoras registradas en el sistema.</p>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-100">
                            ...
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-black text-[#1c607a] uppercase tracking-wider">Nombre</th>
                                <th class="px-6 py-4 text-left text-xs font-black text-[#1c607a] uppercase tracking-wider">Ubicación</th>
                            ...
                            @forelse($incubadoras as $incubadora)
                                <tr class="hover:bg-[#3bb49c]/5 transition-colors duration-200 group">
                                    <td class="px-6 py-4 text-sm font-bold text-gray-800">{{ $incubadora->codigo }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-700">{{ $incubadora->nombre }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-500">{{ $incubadora->ubicacion }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-700">{{ $incubadora->estado->nombre ?? 'Sin estado' }}</td>

                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        <div class="flex items-center justify-center gap-3 opacity-80 group-hover:opacity-100 transition-opacity">
                                            <button
                                                type="button"
                                                data-id="{{ $incubadora->id }}"
                                                data-codigo="{{ $incubadora->codigo }}"
                                                data-nombre="{{ $incubadora->nombre }}"
                                                data-ubicacion="{{ $incubadora->ubicacion }}"
                                                data-descripcion="{{ $incubadora->descripcion }}"
                                                data-estado="{{ $incubadora->estado_incubadora_id }}"
                                                data-action="{{ route('super_admin.incubadoras.update', $incubadora) }}"
                                            ...
                                            <form action="{{ route('super_admin.incubadoras.destroy', $incubadora) }}" method="POST" class="inline m-0">
                                                @csrf
                                                @method('DELETE')
                                                <button
                                                    type="submit"
                                                    onclick="return confirm('¿Deseas eliminar esta incubadora?')"
                ...
                <div x-show="openCreate"
                    ...
                    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden"
                        ...
                        <div class="bg-gradient-to-r from-[#1c607a] to-[#3bb49c] p-6 text-white text-center">
                            <h3 class="text-2xl font-extrabold">Nueva Incubadora</h3>
                            <p class="text-sm opacity-90 mt-1">Registra una nueva incubadora.</p>
                        </div>

                        <form action="{{ route('super_admin.incubadoras.store') }}" method="POST" class="p-8 space-y-5">
                            @csrf

                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Código</label>
                                <input type="text" name="codigo" required class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700">
                            </div>

                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Nombre</label>
                                <input type="text" name="nombre" required class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700">
                            </div>

                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Ubicación</label>
                                <input type="text" name="ubicacion" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700">
                            </div>
                            ...
                            <div>
                                ...
                                <select name="estado_incubadora_id" required class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700 bg-white">
                ...
                <div x-show="openEdit"
                    ...
                    <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden"
                        ...
                        <div class="bg-[#f0f6f6] p-6 border-b border-gray-200">
                            <h3 class="text-2xl font-extrabold text-[#1c607a]">Editar Incubadora</h3>
                            <p class="text-sm text-gray-500 mt-1">Modifica la información de la incubadora.</p>
                        </div>
                        ...
                        <form :action="editAction" method="POST" class="p-8 space-y-5">
                            ...
                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Código</label>
                                <input type="text" name="codigo" x-model="incubadoraToEdit.codigo" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700">
                            </div>

                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Nombre</label>
                                <input type="text" name="nombre" x-model="incubadoraToEdit.nombre" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700">
                            </div>

                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Ubicación</label>
                                <input type="text" name="ubicacion" x-model="incubadoraToEdit.ubicacion" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700">
                            </div>

                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Descripción</label>
                                <textarea name="descripcion" x-model="incubadoraToEdit.descripcion" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700"></textarea>
                            </div>

                            <div>
                                <label class="block text-sm font-bold text-[#1c607a] mb-1">Estado</label>
                                <select name="estado_incubadora_id" x-model="incubadoraToEdit.estado_incubadora_id" class="input-brand w-full rounded-xl py-2.5 px-4 text-gray-700 bg-white">
                                    <option value="">Seleccione un estado</option>
                                    @foreach($estados as $estado)
                                        <option value="{{ $estado->id }}">{{ $estado->nombre }}</option>
                                    @endforeach
