@php
    ...
    */
    ...
    $superAdminGroups = [
        ['key'=>'global','title'=>'Global','description'=>'Administración principal','icon'=>'dashboard','items'=>[
        ...
        ['key'=>'alertas','title'=>'Alertas','description'=>'Incidencias del sistema','icon'=>'alert','items'=>[
        ...
        ['key'=>'incubadoras','title'=>'Incubadoras','description'=>'Dispositivos y asignaciones','icon'=>'incubator','items'=>[
        ...
        ['key'=>'monitoreo','title'=>'Monitoreo','description'=>'Lecturas y automatización','icon'=>'chart','items'=>[
        ...
        ['key'=>'germinacion','title'=>'Germinación','description'=>'Especies, lotes y frascos','icon'=>'seed','items'=>[
        ...
        ['key'=>'biologico','title'=>'Biológico','description'=>'Seguimiento y evidencias','icon'=>'biology','items'=>[
    ...
    */
    ...
    $encargadoItems = [
        ['label'=>'Dashboard','route'=>'encargado.dashboard','active'=>['encargado.dashboard'],'icon'=>'dashboard'],
        ['label'=>'Mis incubadoras','route'=>'encargado.incubadoras.index','active'=>['encargado.incubadoras.*'],'icon'=>'incubator'],
        ['label'=>'Mis alertas','route'=>'encargado.alertas.index','active'=>['encargado.alertas.*'],'icon'=>'alert'],
        ['label'=>'Mis lotes','route'=>'encargado.lotes.index','active'=>['encargado.lotes.*'],'icon'=>'lot'],
        ['label'=>'Mis frascos','route'=>'encargado.frascos.index','active'=>['encargado.frascos.*'],'icon'=>'jar'],
        ['label'=>'Seguimientos de frasco','route'=>'encargado.seguimientos-frasco.index','active'=>['encargado.seguimientos-frasco.*'],'icon'=>'follow-jar'],
        ['label'=>'Seguimientos de lote','route'=>'encargado.seguimientos-lote.index','active'=>['encargado.seguimientos-lote.*'],'icon'=>'follow-lot'],
        ['label'=>'Evidencias','route'=>'encargado.evidencias-lote.index','active'=>['encargado.evidencias-lote.*'],'icon'=>'evidence'],
    ];
...
<div class="microseed-sidebar relative z-50 h-dvh min-h-0 shrink-0"
     x-data="{sidebarOpen:false,openGroup:@js($initialGroup),openSidebar(){this.sidebarOpen=true},closeSidebar(){this.sidebarOpen=false;this.openGroup=null},openGroupOnHover(group){if(this.sidebarOpen)this.openGroup=group},toggleGroup(group){this.sidebarOpen=true;this.openGroup=this.openGroup===group?null:group}}"
    ...
    <style>
        ...
        .microseed-sidebar{--sidebar-font:'Instrument Sans',ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;width:72px;overflow:hidden;border-right:1px solid rgba(203,213,225,.72);background:radial-gradient(circle at 15% 0%,rgba(59,180,156,.08),transparent 22%),linear-gradient(180deg,#fff 0%,#f8fafc 100%);box-shadow:8px 0 30px rgba(15,23,42,.07);font-family:var(--sidebar-font);font-size:14px;font-weight:400;line-height:1.5;font-synthesis:none;text-rendering:optimizeLegibility;-webkit-font-smoothing:antialiased;transition:width .22s cubic-bezier(.4,0,.2,1),box-shadow .22s ease}
        ...
        .sidebar-brand-link{position:relative;z-index:2;width:100%;min-width:0;display:flex;align-items:center;gap:14px}
        .sidebar-collapsed .sidebar-brand-link{justify-content:center}
        .sidebar-brand-logo{width:68px;height:76px;display:flex;align-items:center;justify-content:center;flex-shrink:0;overflow:visible;padding:0;margin:0;border:0;background:transparent;transition:transform .22s ease}
        .sidebar-brand-link:hover .sidebar-brand-logo{transform:translateY(-2px) scale(1.04)}
        ...
        .sidebar-collapsed .sidebar-brand-logo{width:54px;height:64px}
        .sidebar-collapsed .sidebar-brand-logo img{transform:scale(1.12)}
        ...
        .sidebar-menu-container{padding:12px 10px 18px}

        /*
        |--------------------------------------------------------------------------
        | MENÚ DIRECTO DEL ENCARGADO
        |--------------------------------------------------------------------------
        */

        .sidebar-encargado{display:flex;flex-direction:column;gap:5px;width:100%}
        .sidebar-direct-link{position:relative;width:100%;min-height:48px;display:flex;align-items:center;gap:12px;padding:4px 6px;border:1px solid transparent;border-radius:15px;color:#64748b!important;text-decoration:none!important;transition:background .18s ease,color .18s ease,border-color .18s ease,box-shadow .18s ease,transform .18s ease}
        .sidebar-direct-link:hover{color:#216a73!important;background:rgba(236,254,255,.86);border-color:rgba(59,180,156,.12)}
        .sidebar-direct-icon{position:relative;width:40px;height:40px;display:flex;align-items:center;justify-content:center;flex-shrink:0;border-radius:12px;color:#64748b;background:transparent;transition:color .18s ease,background .18s ease,box-shadow .18s ease,transform .18s ease}
        .sidebar-direct-link:hover .sidebar-direct-icon{color:#216a73;background:#ecfeff;transform:translateY(-1px)}
        .sidebar-direct-link-active{color:#1f6f78!important;background:linear-gradient(135deg,rgba(33,106,115,.09),rgba(59,180,156,.12));border-color:rgba(59,180,156,.18);box-shadow:inset 0 1px 0 rgba(255,255,255,.9),0 5px 14px rgba(33,106,115,.06)}
        .sidebar-direct-link-active .sidebar-direct-icon{color:#fff;background:linear-gradient(135deg,#216a73 0%,#3bb49c 100%);box-shadow:0 8px 19px rgba(33,106,115,.23),inset 0 1px 0 rgba(255,255,255,.2)}
        .sidebar-direct-link-active:hover .sidebar-direct-icon{color:#fff;background:linear-gradient(135deg,#1c626b,#32a890)}
        .sidebar-direct-content{min-width:0;flex:1;display:flex;align-items:center;justify-content:space-between;gap:8px;padding-right:5px}
        .sidebar-direct-label{display:block;min-width:0;overflow:hidden;color:inherit;font-size:12px;font-weight:650;line-height:1.25;text-overflow:ellipsis;white-space:nowrap}
        .sidebar-direct-dot{width:7px;height:7px;flex-shrink:0;border-radius:9999px;background:#3bb49c;box-shadow:0 0 0 4px rgba(59,180,156,.12)}
        .sidebar-collapsed .sidebar-direct-link{justify-content:center;min-height:48px;padding:4px}
        .sidebar-collapsed .sidebar-direct-icon{width:40px;height:40px}
        .sidebar-collapsed .sidebar-menu-container{padding-left:10px;padding-right:10px}
        ...
        */
        ...
        @media(max-height:720px){
            ...
            .sidebar-brand-logo{width:56px;height:64px}
            .sidebar-collapsed .sidebar-brand-logo{width:48px;height:56px}
            .sidebar-brand-name{font-size:17px}
            .sidebar-brand-description{margin-top:5px;font-size:9px}
            .sidebar-menu-container{padding-top:7px}
            .sidebar-direct-link{min-height:42px}
            .sidebar-direct-icon{width:36px;height:36px}
            .sidebar-collapsed .sidebar-direct-link{min-height:42px}
            .sidebar-collapsed .sidebar-direct-icon{width:36px;height:36px}
        ...
        <nav class="sidebar-main-scroll min-h-0 flex-1">
            ...
            <div class="sidebar-menu-container">
                ...
                @if($isEncargado)
                    ...
                    {{-- MENÚ DIRECTO DEL ENCARGADO --}}
                    <div class="sidebar-encargado">

                        @foreach($encargadoItems as $item)
                ...
                @elseif($isSuperAdmin)
                    ...
                    @foreach($menuGroups as $group)
                        ...
                        <div class="sidebar-group"
                            ...
                            <div x-show="sidebarOpen && openGroup === @js($group['key'])"
                                 x-cloak
                                 x-collapse
                    ...
                    <div class="sidebar-group"
                        ...
                        <button type="button"
                            ...
                            <span x-show="sidebarOpen" x-cloak x-transition.opacity.duration.140ms class="sidebar-group-content">
                                ...
                                <svg class="sidebar-chevron"
                                     :class="openGroup === 'reportes' ? 'rotate-180' : ''"
                                     fill="none"
                                     stroke="currentColor"
                                     viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
