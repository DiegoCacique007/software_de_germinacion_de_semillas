...
use App\Services\MicroclimaAutomationService;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;

class SensorLecturaController extends Controller
{
    public function store(Request $request,MicroclimaAlertService $alertService,MicroclimaAutomationService $automationService)
    {
        ...
        try{
            ...
            return response()->json(['ok'=>false,'message'=>'Error interno al registrar la lectura.'],500);
        }
        ...
        $automatizacion=['evaluada'=>false];

        try{
            $automatizacion=$automationService->procesar($lectura);
        }catch(\Throwable $e){
            Log::error('Error al ejecutar automatización de microclima',[
                'lectura_microclima_id'=>$lectura->id,
                'incubadora_id'=>$lectura->incubadora_id,
                'error'=>$e->getMessage(),
            ]);
            ...
            $automatizacion=[
                'evaluada'=>false,
                'error'=>'No fue posible ejecutar la automatización.',
            ];
        }
