...
use App\Models\Lote;
use Database\Seeders\AlertasConfigSeeder;
...
class SensorLecturaAlertasTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->seed([
            MicroseedConfigSeeder::class,
            AlertasConfigSeeder::class,
        ]);

        $this->incubadoraId = DB::table('incubadoras')->value('id');

        $posicion = DB::table('posiciones_incubadora')->insertGetId([
            'incubadora_id' => $this->incubadoraId,
            'numero_posicion' => 1,
        ]);

        $especie = DB::table('especies')->insertGetId([
            'nombre_comun' => 'Especie de prueba',
        ]);

        $estado = DB::table('estados_lote')->insertGetId([
            'clave' => 'prueba',
            'nombre' => 'Prueba',
        ]);

        $this->loteId = DB::table('lotes')->insertGetId([
            'posicion_incubadora_id' => $posicion,
            'especie_id' => $especie,
            'estado_lote_id' => $estado,
            'codigo_lote' => 'L-1',
    ...
    {
        return $this->withHeader('X-SENSOR-TOKEN', 'sensor-test-only')
            ->postJson('/api/sensores/lecturas', array_replace([
                'incubadora_id' => $this->incubadoraId,
                'temperatura' => 25,
                'humedad' => 70,
            ], $datos));
    ...
    {
        $response = $this->lectura(compact('temperatura', 'humedad'));

        $response->assertCreated()
            ->assertJsonPath('ok', true)
            ->assertJsonPath('alertas.evaluadas', true)
        ...
        foreach (Alerta::all() as $alerta) {
            ...
            $this->assertSame($response->json('data.id'), $alerta->lecturaMicroclima->id);
            $this->assertSame($this->incubadoraId, $alerta->incubadora_id);
            $this->assertSame('pendiente', $alerta->estado->clave);
            $this->assertNull($alerta->atendida_por);
            $this->assertNull($alerta->fecha_atencion);
            $this->assertNull($alerta->fecha_resolucion);
        }
    }

    public function test_no_duplica_y_resuelve_alerta_al_volver_al_rango(): void
    {
        $lecturaOrigen = $this->lectura(['temperatura' => 35])
            ->assertCreated()
            ->assertJsonPath('alertas.generadas', 1)
            ->json('data.id');

        $this->lectura(['temperatura' => 36])
            ->assertCreated()
            ->assertJsonPath('alertas.generadas', 0);
        ...
        $alerta = Alerta::with('estado')->firstOrFail();

        $this->assertSame($lecturaOrigen, $alerta->lectura_microclima_id);
        $this->assertSame('resuelta', $alerta->estado->clave);
        $this->assertNotNull($alerta->fecha_resolucion);
    }

    public function test_alerta_atendida_sigue_activa_y_no_duplica(): void
    {
        $this->lectura(['temperatura' => 35])->assertJsonPath('alertas.generadas', 1);

        $alerta = Alerta::firstOrFail();
        $estadoAtendida = EstadoAlerta::where('clave', 'atendida')->firstOrFail();

        $alerta->update([
            'estado_alerta_id' => $estadoAtendida->id,
            'fecha_atencion' => now(),
        ]);
        ...
        $this->assertDatabaseCount('alertas', 1);
        $this->assertSame('atendida', $alerta->fresh()->estado->clave);
    ...
    public function test_alerta_resuelta_permite_nueva_incidencia(): void
    {
        $this->lectura(['temperatura' => 35])->assertJsonPath('alertas.generadas', 1);

        $primera = Alerta::firstOrFail();

        $this->lectura()
            ->assertCreated()
            ->assertJsonPath('alertas.generadas', 0);

        $this->assertSame('resuelta', $primera->fresh()->estado->clave);

        $this->lectura(['temperatura' => 35])
            ->assertCreated()
            ->assertJsonPath('alertas.generadas', 1);

        $this->assertDatabaseCount('alertas', 2);

        $ultima = Alerta::latest('id')->firstOrFail();

        $this->assertNotSame($primera->id, $ultima->id);
        $this->assertSame('pendiente', $ultima->estado->clave);
        $this->assertNull($ultima->fecha_resolucion);
    }

    public function test_lectura_normal_resuelve_temperatura_y_humedad_activas(): void
    {
        $this->lectura(['temperatura' => 35, 'humedad' => 50])
            ->assertCreated()
            ->assertJsonPath('alertas.generadas', 2);
        ...
        foreach (Alerta::with('estado')->get() as $alerta) {
            $this->assertSame('resuelta', $alerta->estado->clave);
            $this->assertNotNull($alerta->fecha_resolucion);
        }
    }
    ...
    {
        $otra = DB::table('incubadoras')->insertGetId([
            'codigo' => 'OTRA',
            'nombre' => 'Otra',
    ...
    {
        $this->withHeader('X-SENSOR-TOKEN', 'sensor-test-only')
            ->postJson('/api/sensores/lecturas', [
                'temperatura' => 25,
                'humedad' => 70,
            ])
            ->assertUnprocessable()
            ->assertJsonValidationErrors('incubadora_id');

        $this->lectura(['incubadora_id' => 999999])
            ->assertUnprocessable();
    ...
    public function test_token_incorrecto_ausente_o_sin_configurar(): void
    {
        ...
        $this->withHeader('X-SENSOR-TOKEN', 'incorrecto')
            ->postJson('/api/sensores/lecturas', [])
            ->assertUnauthorized();
    ...
    public function test_fallo_de_configuracion_conserva_lectura_sin_exponer_excepcion(): void
    {
        ...
        $this->lectura(['temperatura' => 35])
            ->assertCreated()
            ->assertJsonPath('ok', true)
            ->assertJsonPath('alertas.evaluadas', false)
            ->assertJsonPath('alertas.generadas', 0)
    ...
    public function test_fallo_en_otro_lote_revierte_solo_alertas(): void
    {
        ...
        $lote->especie_id = DB::table('especies')->insertGetId([
            'nombre_comun' => 'Sin condición',
        ]);
        $lote->save();
    ...
    public function test_niveles_conservan_umbrales(): void
    {
        $estadoResuelta = EstadoAlerta::where('clave', 'resuelta')->firstOrFail();

        foreach ([31 => 'bajo', 32 => 'medio', 35 => 'alto'] as $temperatura => $nivel) {
            $this->lectura(['temperatura' => $temperatura])
                ->assertJsonPath('alertas.generadas', 1);

            $alerta = Alerta::latest('id')->firstOrFail();
        ...
        $this->assertDatabaseCount('alertas', 3);
    }
